...
const Smartphone = () => {
  ...
  const [screenContent, setScreenContent] = useState("Welcome to the USSD Simulator. Tap to enter a code (e.g., *384#)."); // Screen display
  ...
  const handleSend = async () => {
    ...
    try {
      let response;
      if (isFirstRequest && (input === "*384#" || (input.startsWith("*384*") && input.endsWith("#")))) {
       
        response = await axios.post(`${BASE_URL}/api/v1/user/userResponse/collect`, {}, {
          params: {
            sessionId,
            serviceCode: input, 
            text: "",          
